...
def precision_at_k(recommender: ContentBasedRecommender,
                   test_movies: pd.DataFrame, k: int = 10) -> float:
...
def ndcg_at_k(recommender: ContentBasedRecommender,
              test_movies: pd.DataFrame, k: int = 10) -> float:
...
def coverage(recommender: ContentBasedRecommender,
             test_movies: pd.DataFrame, catalog_size: int,
             k: int = 10) -> float:
...
def intra_list_diversity(recommender: ContentBasedRecommender,
    ...
    for _, row in test_movies.iterrows():
        ...
        # Get indices of recommended movies in the recommender's DataFrame
        ...
        # Pairwise cosine similarity from precomputed matrix
...
def novelty(recommender: ContentBasedRecommender,
            test_movies: pd.DataFrame,
...
def grid_search_weights(df: pd.DataFrame, test_movies: pd.DataFrame,
    ...
    for i, combo in enumerate(combos):
        ...
        result_entry = {'weights': dict(zip(fields, combo)),
...
if __name__ == '__main__':
    ...
    # Sample 100 test movies with enough votes
