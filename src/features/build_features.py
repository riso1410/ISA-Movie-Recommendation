...
def build_features(smd):
    ...
    smd["genres_str"] = smd["genres"].apply(
        lambda x: " ".join(str(g).lower().replace(" ", "") for g in x)
    ...
    smd["collection"] = (
        smd["collection"]
        .fillna("")
        .apply(lambda x: str(x).lower().replace(" ", "") if x else "")
        if "collection" in smd.columns
        else ""
    )

    director_3x = smd["director_str"].apply(lambda d: " ".join([d] * 3) if d else "")
    smd["soup"] = (
        smd["overview_clean"]
        + " "
        + smd["genres_str"]
